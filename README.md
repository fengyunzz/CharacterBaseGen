...
当前这套保护能挡住“大部分误提交”，但不是绝对保险，边界要说明白：

- 已被 Git 跟踪过的私有文件，不会因为后来写进 `.gitignore` 就自动消失。
- 如果你把真实素材放到仓库根目录的未忽略位置，还是可能被提交。
- README、issue、commit message、截图和对话里的文字说明，也可能泄露项目名或角色信息。
...
- `git status -sb` 里没有真实项目目录和图片产物。
- `git ls-files` 里没有 `projects/<真实项目名>/`、`archive/<真实项目名>/`。
- `rg --files -uu` 的命中如果是模板或 `.gitkeep` 之外的真实素材，就不要推送。
